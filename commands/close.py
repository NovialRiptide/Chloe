...
class sessions(commands.Cog):
    ...
    async def close(self, ctx):
        ...
        if int(ctx.channel.topic) == ctx.author.id or server_management_role in ctx.author.roles:
            ...
            if is_a_session_channel and ctx.channel.category_id == OCCUPIED_CATEGORY_ID:
                ...
                try: await member.remove_roles(ctx.channel.guild.get_role(IN_SESSION_ROLE))
                except: pass
                await ctx.channel.edit(category=dormant_category, sync_permissions=True, topic="")
                await ctx.channel.send("**This channel has been marked as dormant.**\nThis channel is not meant to be in use.")
        else:
